...
class TopCategoryController extends ResponseController
{
    // store top category
    public function store(Request $request)
    {
        ...
        $path = 'top_categories'; // Example path, adjust as needed
        $categoryCarouselUrl = S3Service::uploadSingle($file, $path);



        DB::table(table: 'top_categories')->insert([
        ...
        return $this->sendResponse('Category  created successfully', 'Category Carousel created successfully');
        }catch(Exception $e){
            return $this->sendError('Something went wrong', [], 500);
        }
    }

    // update top_categories
    public function update(Request $request, $id){
        try{
            $findCategoriesCarousel = DB::table('top_categories')->where('id', $id)->first();
           ...
           if($request->image){
            ...
            $path = 'top_categories'; // Example path, adjust as needed
            $categoryCarouselUrl = S3Service::uploadSingle($file, $path);
            DB::table('top_categories')->where('id', $id)->update([
    ...
    public function delete($id){
        try{
           ...
           if($findCategoriesCarousel->image){
            ...
            DB::table('top_categories')->where('id', $id)->delete();
    ...
    public function getAll(){
        try{
            $topCategories = DB::table('top_categories')->
            leftJoin('categories', 'top_categories.categoryId', '=', 'categories.id')
            ->leftJoin('sub_categories', 'top_categories.subCategoryId', '=', 'sub_categories.id')->
            select(
                'top_categories.id as id',
                'top_categories.image as image',
